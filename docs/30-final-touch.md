...
**`참고`** 정규표현식의 작성 및 테스트를 위해서는 [Regex010](https://regex101.com/) 사이트를 이용하자. 필자가 본 정규표현식 툴 중 최고이다.

이미지 응답을 만들기 위해서 intervention/image 패키지를 이용할 것이다. composer 로 설치하고, [매뉴얼](http://image.intervention.io/getting_started/installation#laravel)에 따라 config/app 파일에 서비스프로바이더, Facade도 추가하자.
